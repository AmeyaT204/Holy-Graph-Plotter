...
// New in week 9: 0 = Phong (smooth, interpolated normal),
//                1 = Flat  (one normal per triangle, from screen-space
//                           derivatives of position -- no extra vertex
//                           data or second VBO needed).
...
void main()
{
    ...
    vec3 N;
    if (uShadeMode == 1)
        N = normalize(cross(dFdx(fPos), dFdy(fPos)));
    else
        N = normalize(fNormal);
